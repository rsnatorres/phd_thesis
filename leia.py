''' LeIA - Lexicon for Adaptive Inference

This project is a fork of the lexicon and sentiment analysis tool VADER (Valence Aware Dictionary and sEntiment Reasoner), adapted for Portuguese texts.

VADER Author: C.J. Hutto
...
from config import path_config, file_config
...
NEGATE = [t.strip() for t in open(os.path.join(path_config.path_lexicons, file_config.negate_file))]

# Booster/dampener 'intensifiers' or 'degree adverbs' (Portuguese)
boosters = []
for l in open(os.path.join(path_config.path_lexicons, file_config.booster_file)):
    parts = l.strip().split(' ')
...
class SentimentIntensityAnalyzer(object):
    ...
    def __init__(self, lexicon_file=os.path.join(path_config.path_lexicons, file_config.vader_file), emoji_lexicon=os.path.join(path_config.path_lexicons, file_config.emoji_file)):
